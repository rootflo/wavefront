...
class RedshiftConnector:
    ...
    def get_connection(self, timeout=300):
        connection = None
        try:
            connection: Connection = redshift_connector.connect(
    ...
    def __close_connection(self, connection: Connection):
        try:
            if connection:
                connection.close()
        except Exception as e:
    ...
    def execute_query(
        self, query: str, parameters: dict | None = None, connection: Connection = None
    ):
        try:
            if self.config.read_only:
