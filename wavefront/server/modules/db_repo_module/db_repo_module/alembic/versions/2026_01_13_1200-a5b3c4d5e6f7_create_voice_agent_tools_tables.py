...
def upgrade() -> None:
    ...
    # Check if voice_agent_tools table already exists
    inspector = sa.inspect(connection)
    if 'voice_agent_tools' in inspector.get_table_names():
        print("Table 'voice_agent_tools' already exists, skipping creation")
        return

    # Create voice_agent_tools table
    op.create_table(
        'voice_agent_tools',
        ...
        sa.Column(
            'parameter_schema', postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
    ...
    op.create_index(
        op.f('ix_voice_agent_tools_name'), 'voice_agent_tools', ['name'], unique=True
    )

    # Create voice_agent_tool_associations table
    op.create_table(
        ...
        sa.Column(
            'config_overrides', postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        ...
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('voice_agent_id', 'tool_id', name='uq_voice_agent_tool'),
    )
    op.create_index(
