...
export class AppService {
  ...
  async getAppById(appId: string): Promise<AppResponse> {
    const response: IApiResponse<AppData> = await this.http.get(
      `/v1/apps/${appId}`
    );
    return response;
  }
  ...
  async createApp(appData: CreateAppRequest): Promise<AppResponse> {
    const response: IApiResponse<AppData> = await this.http.post(
      "/v1/apps",
      appData
    );
    return response;
  ...
  async deleteApp(appId: string): Promise<DeleteAppResponse> {
    const response: IApiResponse<DeleteAppData> = await this.http.delete(
      `/v1/apps/${appId}`
    );
    return response;
  }
  ...
  async getAppStatus(appId: string): Promise<AppStatusResponse> {
    const response: IApiResponse<AppStatusData> = await this.http.get(
