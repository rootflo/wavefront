...
export class DataPipelineService {
  ...
  async listPipelines(status?: PipelineStatus): Promise<PipelinesListResponse> {
    ...
    const response: IApiResponse<PipelinesListData> = await this.http.get(
      `/v1/:appId/floware/v1/pipelines`,
      {
        params,
      }
  ...
  async getPipeline(pipelineId: string): Promise<PipelineResponse> {
    const response: IApiResponse<PipelineData> = await this.http.get(
