...
export class AuthenticatorService {
  ...
  async createAuthenticator(data: CreateAuthenticatorRequest): Promise<AuthenticatorResponse> {
  ...
  async getAuthenticator(authId: string): Promise<AuthenticatorDetailResponse> {
    const response: IApiResponse<Authenticator> = await this.http.get(`/v1/:appId/floware/v1/authenticators/${authId}`);
    return response;
  }

  async getAllAuthenticators(): Promise<AuthenticatorListResponse> {
    const response: IApiResponse<AuthenticatorListData> = await this.http.get(`/v1/:appId/floware/v1/authenticators`);
    return response;
  }

  async updateAuthenticator(authId: string, data: UpdateAuthenticatorRequest): Promise<AuthenticatorResponse> {
  ...
  async enableAuthenticator(authId: string): Promise<AuthenticatorEnableDisableResponse> {
    const response: IApiResponse<AuthenticatorEnableDisableData> = await this.http.post(
      `/v1/:appId/floware/v1/authenticators/${authId}/enable`
    );
    return response;
  }

  async disableAuthenticator(authId: string): Promise<AuthenticatorEnableDisableResponse> {
    const response: IApiResponse<AuthenticatorEnableDisableData> = await this.http.post(
