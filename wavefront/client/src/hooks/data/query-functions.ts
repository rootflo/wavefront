...
const getDatasourceQueryFn = async (
  datasourceId: string
): Promise<Datasource | null> => {
  const response = await floConsoleService.datasourcesService.getDatasource(
    datasourceId
  );
  if (response.data?.data) {
    ...
    return {
      ...
      config:
        typeof responseData.config === "string"
          ? JSON.parse(responseData.config)
          : responseData.config || {},
...
const getAllYamlsQueryFn = async (datasourceId: string): Promise<Yaml[]> => {
  const response = await floConsoleService.datasourcesService.getAllYamls(
    datasourceId
  );
...
const readYamlQueryFn = async (
  datasourceId: string,
  yamlId: string
): Promise<ReadYamlData | null> => {
  const response = await floConsoleService.datasourcesService.readYaml(
    datasourceId,
    yamlId
  );
  if (response.data?.data) {
...
const getDatasourceResourcesQueryFn = async (
  datasourceId: string
): Promise<string[]> => {
  const response =
    await floConsoleService.datasourcesService.getDatasourceResources(
      datasourceId
    );
...
const getAuthenticatorsQueryFn = async (): Promise<Authenticator[]> => {
  const response =
    await floConsoleService.authenticatorService.getAllAuthenticators();
  if (
    response.data?.meta?.status === "success" &&
    response.data.data?.authenticators
  ) {
...
const getAuthenticatorQueryFn = async (
  authId: string
): Promise<Authenticator | null> => {
  const response =
    await floConsoleService.authenticatorService.getAuthenticator(authId);
  if (response.data?.data) {
    return response.data.data;
  }
  return null;
...
const getLLMConfigsQueryFn = async (): Promise<LLMInferenceConfig[]> => {
  const response =
    await floConsoleService.llmInferenceService.listAllLLMConfigs();
  if (
    response.data?.meta?.status === "success" &&
...
const getModelsQueryFn = async (): Promise<ModelData[]> => {
  const response =
    await floConsoleService.modelInferenceService.listAllModels();
  if (response.data?.meta?.status === "success" && response.data.data?.data) {
...
const getModelQueryFn = async (modelId: string): Promise<ModelData | null> => {
  const response = await floConsoleService.modelInferenceService.getModel(
    modelId
  );
  if (response.data?.meta?.status === "success" && response.data.data?.data) {
    return response.data.data.data;
...
const getKnowledgeBasesQueryFn = async (): Promise<KbData[]> => {
  const response =
    await floConsoleService.knowledgeBaseService.listKnowledgeBases();
  if (
    response.data?.meta?.status === "success" &&
...
const getKnowledgeBaseQueryFn = async (
  kbId: string
): Promise<KbData | null> => {
  const response =
    await floConsoleService.knowledgeBaseService.getKnowledgeBase(kbId);
  if (response.data?.data) {
    return response.data.data;
  }
  return null;
...
const getKnowledgeBaseDocumentsQueryFn = async (
  kbId: string
): Promise<DocumentData[]> => {
  const response =
    await floConsoleService.knowledgeBaseService.listKnowledgeBaseDocuments(
      kbId
    );
...
const getKnowledgeBaseInferencesQueryFn = async (
  kbId: string
): Promise<InferenceData[]> => {
  const response =
    await floConsoleService.knowledgeBaseService.listInferencesForKnowledgeBase(
      kbId
    );
  if (response.data?.data?.resources) {
    return response.data.data.resources;
...
const getWorkflowsQueryFn = async (
  namespace?: string
): Promise<WorkflowListItem[]> => {
  const response = await floConsoleService.workflowService.listWorkflows(
    namespace
  );
  if (response.data?.meta?.status === "success" && response.data.data?.data) {
...
const getWorkflowPipelinesQueryFn = async (): Promise<
  WorkflowPipelineListItem[]
> => {
  const response =
    await floConsoleService.workflowService.listWorkflowPipelines();
  if (
    response.data?.meta?.status === "success" &&
    response.data.data?.workflow_pipelines
  ) {
...
const getVoiceAgentsQueryFn = async (): Promise<VoiceAgent[]> => {
  const response =
    await floConsoleService.voiceAgentService.listAllVoiceAgents();
  if (
    response.data?.meta?.status === "success" &&
    response.data.data?.voice_agents
  ) {
...
const getTtsConfigsQueryFn = async (): Promise<TtsConfig[]> => {
  ...
  if (
    response.data?.meta?.status === "success" &&
    response.data.data?.tts_configs
  ) {
...
const getSttConfigsQueryFn = async (): Promise<SttConfig[]> => {
  ...
  if (
    response.data?.meta?.status === "success" &&
    response.data.data?.stt_configs
  ) {
...
const getTelephonyConfigsQueryFn = async (): Promise<TelephonyConfig[]> => {
  const response =
    await floConsoleService.telephonyConfigService.listAllTelephonyConfigs();
  if (
    response.data?.meta?.status === "success" &&
    response.data.data?.telephony_configs
  ) {
...
const getVoiceAgentQueryFn = async (
  agentId: string
): Promise<VoiceAgent | null> => {
  const response = await floConsoleService.voiceAgentService.getVoiceAgent(
    agentId
  );
  if (response.data?.data) {
    return response.data.data;
...
const getTtsConfigQueryFn = async (
  configId: string
): Promise<TtsConfig | null> => {
  const response = await floConsoleService.ttsConfigService.getTtsConfig(
    configId
  );
  if (response.data?.data) {
    return response.data.data;
...
const getSttConfigQueryFn = async (
  configId: string
): Promise<SttConfig | null> => {
  const response = await floConsoleService.sttConfigService.getSttConfig(
    configId
  );
  if (response.data?.data) {
    return response.data.data;
...
const getTelephonyConfigQueryFn = async (
  configId: string
): Promise<TelephonyConfig | null> => {
  const response =
    await floConsoleService.telephonyConfigService.getTelephonyConfig(configId);
  if (response.data?.data) {
    return response.data.data;
  }
  return null;
...
const getToolsQueryFn = async (): Promise<ToolDetails[]> => {
  ...
  if (
    response.data?.data?.data?.tool_details &&
    Array.isArray(response.data.data.data.tool_details)
  ) {
...
const getMessageProcessorsQueryFn = async (): Promise<
  MessageProcessorListItem[]
> => {
  const response =
    await floConsoleService.messageProcessorService.listMessageProcessors();
  if (
    response.data?.data?.processors &&
    Array.isArray(response.data.data.processors)
  ) {
...
const getMessageProcessorQueryFn = async (
  processorId: string
): Promise<MessageProcessor | null> => {
  const response =
    await floConsoleService.messageProcessorService.getMessageProcessor(
      processorId
    );
...
const getApiServiceQueryFn = async (
  serviceId: string
): Promise<ApiServiceItem | null> => {
  const response = await floConsoleService.apiServiceService.getApiService(
    serviceId
  );
  if (response.data?.data) {
    return response.data.data;
...
const getLLMConfigQueryFn = async (
  configId: string
): Promise<LLMInferenceConfig | null> => {
  const response = await floConsoleService.llmInferenceService.getLLMConfig(
    configId
  );
  if (response.data?.data) {
    return response.data.data;
...
const getPipelinesQueryFn = async (
...
const getPipelineQueryFn = async (
  pipelineId: string
...
const getPipelineFilesQueryFn = async (
  pipelineId: string
