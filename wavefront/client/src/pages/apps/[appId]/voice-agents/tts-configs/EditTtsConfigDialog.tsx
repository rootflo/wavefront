...
interface EditTtsConfigDialogProps {
  isOpen: boolean;
...
const EditTtsConfigDialog: React.FC<EditTtsConfigDialogProps> = ({ isOpen, onOpenChange, config, onSuccess }) => {
  ...
  const form = useForm<UpdateTtsConfigInput>({
    resolver: zodResolver(updateTtsConfigSchema),
    defaultValues: {
  ...
  useEffect(() => {
    if (isOpen && config) {
      ...
      setParameters(mergeParameters('tts', config.provider, config.parameters));
    }
  }, [isOpen, config, form]);
  ...
  useEffect(() => {
    if (isOpen && watchedProvider && watchedProvider !== config.provider) {
      setParameters(mergeParameters('tts', watchedProvider, null));
    }
  ...
  const onSubmit = async (data: UpdateTtsConfigInput) => {
    setLoading(true);
    try {
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="e.g., OpenAI TTS English" maxLength={100} {...field} />
                    </FormControl>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="e.g., alloy, echo, fable" {...field} />
                  </FormControl>
                  <FormDescription>Voice identifier for the selected provider</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
...
export default EditTtsConfigDialog;
